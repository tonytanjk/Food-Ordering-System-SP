...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_order_id'])) {
    ...
    $query = "SELECT total_amount, status, UNIX_TIMESTAMP(order_date) AS order_time 
...
<head>
    ...
    <style>
        ...
        .message { text-align: center; margin: 10px 0; padding: 10px; border-radius: 5px; font-size: 16px; }
        .success { color: green; background-color: #e8f5e9; }
        .error { color: red; background-color: #fbe9e7; }
        table { width: 100%; border-collapse: collapse; }
        table th, table td { border: 1px solid #ddd; padding: 10px; text-align: left; }
        table th { background-color: #f4f4f4; }
        table tbody tr:nth-child(even) { background-color: #f9f9f9; }
        .cancel-btn { padding: 5px 10px; background-color: #ff4d4d; color: white; border: none; border-radius: 4px; cursor: pointer; }
